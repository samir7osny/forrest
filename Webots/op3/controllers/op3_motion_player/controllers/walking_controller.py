...
class WalkingController(_controller):
    def __init__(self, accuracy=1):
        ...
        t, right_foot_height, left_foot_height, pelvis_side_displacement, right_foot_forward_displacement, left_foot_forward_displacement, pelvis_forward_displacement = pattern_generator.generate_full_pattern(number_of_steps=2)
    ...
    def get_step(self):
        left_foot_z_value = self.data['left_foot_z_values'][min(self.current_time, len(self.data['t']) - 1)]
        left_foot_x_value = self.data['left_foot_x_values'][min(self.current_time, len(self.data['t']) - 1)]
        pelvis_x_value = self.data['pelvis_x_values'][min(self.current_time, len(self.data['t']) - 1)]
        right_foot_z_value = self.data['right_foot_z_values'][min(self.current_time, len(self.data['t']) - 1)]
        right_foot_x_value = self.data['right_foot_x_values'][min(self.current_time, len(self.data['t']) - 1)]
        pelvis_y_value = self.data['pelvis_y_values'][min(self.current_time, len(self.data['t']) - 1)]
