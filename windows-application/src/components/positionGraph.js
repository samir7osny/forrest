...
export function PositionGraph(props) {
    ...
    useEffect(() => {
        ...
        robot_paths.exit()
            ...
            .attr('opacity', 0)
            .remove()
        let robot_paths_enter = robot_paths.enter().append('path').attr('class', 'robot')
        let new_robot_paths = robot_paths_enter.merge(robot_paths)
            .attr('d', d=>line(d))
            // .attr('stroke', (d, i)=>i == 0 ? 'black' : 'red')
        var oldTotalLengths = new_robot_paths.nodes().map(ele => parseFloat(ele.getAttribute('old_length'))||0)
        var totalLengths = new_robot_paths.nodes().map(ele => ele.getTotalLength())
        new_robot_paths.attr("stroke-dasharray", (d, i)=>totalLengths[i])
            .attr("stroke-dashoffset", (d, i)=>totalLengths[i] - oldTotalLengths[i])
            .transition(t)
            .attr("stroke-dashoffset", 0)
            .attr("old_length", (d, i)=>totalLengths[i])
        ...
        // ############################################ Robot Checkpoints ############################################
        let robot_bullets = svg.selectAll('circle.robot_bullet')
            .data(robot_path)
        robot_bullets.exit()
            .transition(t)
            .attr('r', 0)
            .remove()
        let robot_bullets_enter = robot_bullets.enter()
            .append('circle')
            .attr('r', 0)
            .attr('class', 'robot_bullet')
            .attr('cx', (d, i)=>xScale((robot_path[i-1]||robot_path[i])[0]))
            .attr('cy', (d, i)=>yScale((robot_path[i-1]||robot_path[i])[1]))
        robot_bullets_enter.merge(robot_bullets)
            .transition(t)
            .attr('cx', d=>xScale(d[0]))
            .attr('cy', d=>yScale(d[1]))
        ...
        // ############################################ Robot Target ############################################
        let targets = svg.selectAll('circle.target')
            .data(target ? [target] : [])
        targets.exit()
            .transition(t)
            .attr('r', 0)
            .remove()
        let targets_enter = targets.enter()
            .append('circle')
            .attr('r', 0)
            .attr('class', 'target')
            .attr('cx', (d, i)=>xScale(last_target[0]))
            .attr('cy', (d, i)=>yScale(last_target[1]))
        targets_enter.merge(targets)
            .transition(t)
            .attr('cx', d=>xScale(d[0]))
            .attr('cy', d=>yScale(d[1]))
            .attr('r', robot_bullet_size)
